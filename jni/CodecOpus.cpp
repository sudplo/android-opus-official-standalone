...
int CodecOpus::encoderSetDtx(int dtx) {
    int ret = checkForNull("encoderSetDtx", true);
    if (ret < 0) return ret;
    return opus_encoder_ctl(encoder, OPUS_SET_DTX(dtx));
}

int CodecOpus::encoderSetFec(int fec) {
    int ret = checkForNull("encoderSetFec", true);
    if (ret < 0) return ret;
    opus_encoder_ctl(encoder, OPUS_SET_INBAND_FEC(fec));
    return opus_encoder_ctl(encoder, OPUS_SET_PACKET_LOSS_PERC(10));
}
